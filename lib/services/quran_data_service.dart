    ...
    try {
      ...
      for (int i = 1; i <= 114; i++) {
        ...
        String chapterDataPath =
            'assets/data/XML/Arabic/Quran_Arabic_$chapterFileName.xml';
  ...
  static Future<List<Verse>> getVerses(int chapterNumber) async {
    try {
      final arabicData = await rootBundle.loadString(
          'assets/data/XML/Arabic/Quran_Arabic_${chapterNumber.toString().padLeft(3, '0')}.xml');
      final englishData = await rootBundle.loadString(
          'assets/data/XML/English/Quran_Translation_Shakir_${chapterNumber.toString().padLeft(3, '0')}.xml');
